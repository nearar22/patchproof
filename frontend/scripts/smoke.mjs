...
const id = process.env.DEMO_CASE_ID || "author-safety-demo";
...
async function write(label, functionName, args, intelligent = false) {
  ...
  const status = String(receipt.statusName ?? receipt.status ?? "unknown"), execution = String(receipt.txExecutionResultName ?? receipt.txExecutionResult ?? "unknown");
  console.log(`${label}_STATUS=${status};EXECUTION_RESULT=${execution}`);
  if (!isSuccessful(receipt) || execution !== "FINISHED_WITH_RETURN") throw new Error(`${label} failed`); return hash;
}
